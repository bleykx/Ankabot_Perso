...
-- Définit si le bot doit vendre les [Beignet de Greuvette] par lot de 100 en hôtel de vente (Défaut : false)
-- /!\ Attention : nécessite des Kamas pour payer les taxes
SellMode = false

-- Définit le prix minimum pour la mise en vente des lots de 100 [Beignet de Greuvette] (Défaut : 50000)
MinPrice = 50000
...
function move()
    ...
    if (map:currentArea() == "Astrub") then
        ...
        if (NeedToCraft) then
            map:moveToward(192937984) -- mapid atelier pecheur
            return {
                {map = "192937984", custom = ProcessCraft}, -- mapid interieur atelier pecheur
            }
        end
        ...
        -- Définition du chemin à suivre en fonction du niveau du métier Pêcheur
        if (job:level(36) < 10) then -- si pecheur n'est pas lvl 10 alors on focus le trajet GOUJON
            GATHER = {75, 84}
            return DeplacementsGoujon()
        elseif (job:level(36) >= 10) then
            GATHER = {71, 74, 75, 76, 77, 78, 79, 84}
            return DeplacementsFullPoisson()
        end
    end
end
...
function DeplacementsFullPoisson()
	return {
        { map = "4,-19", path = "top", gather = false, fight = false },  
        { map = "4,-20", path = "top", gather = true, fight = false },  
        { map = "4,-21", path = "top", gather = false, fight = false },  
...
function ProcessBank()
    ...
    if (NeedToReturnBank) then
        ...
        if (SellMode) then
            ...
            global:printSuccess("[Info] Je peux vendre " .. nombreLotsGreuvette .. " lots de [Beignet de Greuvette] !")
        ...
        NbGreuvette = exchange:storageItemQuantity(598)

        global:printSuccess("[Banque] : " .. NbGreuvette .. " [Greuvettes].")

        if (job:level(36) >= 10) then
            global:printSuccess("[Info] Le niveau de votre métier de Pêcheur vous permet de fabriquer des [Beignet de Greuvette] !")
            local NbLotsGreuvette = math.floor(NbGreuvette / 2)

            if (NbLotsGreuvette < 1) then
                global:printError("[Info] Pas assez d'[Greuvettes] disponibles en banque pour fabriquer des [Beignet de Greuvette]")
            else

                local podsAvailable = inventory:podsMax() - inventory:pods()

                local quantiteMaxGreuvetteAPrendre = podsAvailable

                CraftQuantity = NbLotsGreuvette;

                if (NbLotsGreuvette * 2 > quantiteMaxGreuvetteAPrendre) then
                    CraftQuantity = math.floor(quantiteMaxGreuvetteAPrendre / 2)
                end

                global:printSuccess("[Info] Possibilité de création de " .. CraftQuantity .. "x [Beignet de Greuvette]")
                global:printSuccess("[Info] Quantités à retirer de [Greuvette] : " .. CraftQuantity * 2 .. "")

                exchange:getItem(598, CraftQuantity * 2)
                global:delay(2647)
                NeedToCraft = true
            end
        else
            global:printSuccess("[Info] Vous n'avez pas encore atteint le niveau de Pêcheur nécessaire à la fabrication de [Beignet de Greuvette] !")
        end
    end
...
function ProcessCraft()
    ...
    global:leaveDialog()
    global:delay(1258)
    global:printMessage("Je viens de crafter mes objets, je sors de l'atelier ...")
    map:moveToCell(457)
    NeedToCraft = false
