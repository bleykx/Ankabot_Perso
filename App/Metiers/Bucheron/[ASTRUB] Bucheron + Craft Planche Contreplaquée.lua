...
function move()
    ...
    if (map:currentArea() == "Astrub") then
        ...
        if (NeedToCraft) then
            ...
            return {
                {map = "192940042", custom = ProcessCraft},
            }
        end
...
function ProcessBank()
    ...
    if (NeedToReturnBank) then
        ...
        NbFrene = exchange:storageItemQuantity(303)
        NbChataignier = exchange:storageItemQuantity(473)

        global:printSuccess("[Banque] : " .. NbFrene .. " [Frene].")
        global:printSuccess("[Banque] : " .. NbChataignier .. " [Chataignier].")

        if (job:level(2) >= 20) then
            global:printSuccess("[Info] Le niveau de votre métier Bucheron vous permet de fabriquer des [Potion de Souvenir] !")
            local nbLotsFrene = math.floor(NbFrene / 10)
            local nbLotsChataignier = math.floor(NbChataignier / 10)

            if (nbLotsFrene < 1) then
                global:printError("[Info] Pas assez d'[Frene] disponibles en banque pour fabriquer des [Planche Contreplaquée]")
            elseif (nbLotsChataignier < 1) then
                global:printError("[Info] Pas assez de [Chatainier] disponibles en banque pour fabriquer des [Planche Contreplaquée]")
            else

                local podsAvailable = math.floor((inventory:podsMax() - inventory:pods()) / 5)

                local quantiteMaxFrenesAPrendre = math.floor((podsAvailable / 2))
                local quantiteMaxChatainiersAPrendre = podsAvailable - quantiteMaxFrenesAPrendre

                if (nbLotsFrene > nbLotsChataignier) then
                    CraftQuantity = nbLotsChataignier
                elseif (nbLotsFrene < nbLotsChataignier) then
                    CraftQuantity = nbLotsFrene
                end

                if (nbLotsFrene * 10 > quantiteMaxFrenesAPrendre) then
                    CraftQuantity = math.floor(quantiteMaxFrenesAPrendre / 10)
                elseif (nbLotsChataignier * 10 > quantiteMaxChatainiersAPrendre) then
                    CraftQuantity = math.floor(quantiteMaxChatainiersAPrendre / 10)
                end

                global:printSuccess("[Info] Possibilité de création de " .. CraftQuantity .. "x [Planche Contreplaquée]")
...
function ProcessCraft()
    ...
    global:leaveDialog()
    global:delay(1258)
    global:printMessage("Je viens de finir de craft mes objets, je sors de l'atelier ...")
    map:moveToCell(414)
    NeedToCraft = false
